...
- Em thấy dòng thông báo "Hello from JavaScript!" trong console.
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
- Điều này cho thấy JavaScript sẽ tự chạy sau khi tải trang web.
---
...
- Dòng log hiển thị thêm "Xin chào, mình là Vững, năm nay mình 20 tuổi.".
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
-Nếu quên dấu";" thì code vẫn chạy còn quên dấu "+" thì nó sẽ báo lỗi "Uncaught SyntaxError: missing ) after argument list".
...
-console có thể chạy kết quả ra màn hình .
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
-Em ko gặp lỗi cú pháp nào nếu gặp lỗi em sẽ dùng google.
---
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: đúng với dự đoán.
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: đúng với dự đoán.
...
-Khi score>=8 thì in ra giỏi
> - Khi nào chương trình in `"Yếu"`?
-Khi score<5 thì in ra yếu
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):
- Cấu trúc if/else giống như một hệ thống ra quyết định: chương trình kiểm tra điều kiện, nếu đúng thì làm việc A, nếu sai thì làm việc B hoặc kiểm tra điều kiện khác.
---
...
>   - Tên hàm: Dùng để gọi hàm thực thi.
>   - Tham số (parameters): Là các giá trị đầu vào mà hàm nhận để xử lý.
>   - Thân hàm (body): Chứa các câu lệnh mà hàm sẽ thực hiện.
...
- Giúp tái sử dụng đoạn code nhiều lần,làm chương trình gọn gàng, dễ đọc hơn,dễ bảo trì và sửa lỗi.
---
...
- Phần khiến em khó hiểu nhất là return. Ban đầu em chưa hiểu rõ giá trị trả về sẽ được dùng như thế nào và khác gì với việc chỉ dùng console.log() trong hàm.
...
-Tìm và lấy một phần tử trong trang HTML có id là "status".
> - Sự kiện `"click"` xảy ra khi nào?
- Khi ta click vào nút chào.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
- Nó hiển thị nội dung"Xin chào! Đây là nội dung được thay đổi bằng JavaScript."
---
...
VD: document.body.style.color = "blue"; là đổi màu chữ toàn trang.
---
...
-Sự kiện input khi xảy ra khi giá trị của ô nhập thay đổi gõ, xoá, dán nội dung.
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
-Dòng greeting hiển thị "Xin chào, !'".
...
> -DOM là mô hình giúp JavaScript nhìn thấy và chỉnh sửa các phần tử trong trang web.
...
### 6.1. Nhìn nhanh jQuery (so sánh với JS thuần)
  ...
  -Cả hai đoạn code đều dùng để gắn sự kiện click cho nút btnHello và hiển thị thông báo (alert) khi người dùng nhấn nút.
  > - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
  -jquery ngắn gọn hơn so với javascript.
  > - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
  >   1. Giúp thao tác DOM dễ dàng và ngắn gọn hơn so với JavaScript thuần.
  >   2. Giúp xử lý sự kiện, hiệu ứng (animation) và tương tác người dùng nhanh chóng hơn.
...
-em tò mò javascrpit có thể làm thêm những j cho trang web.
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
-em muốn tìm hiểu về thêm mini game cho trang web.
